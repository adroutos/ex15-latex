...
\usepackage{tikz} % para fazer o automato
\usepackage{amsmath} % para simobolos matematicos
\usepackage{float} % para forcar figure a ficar onde quer
...
\begin{figure}[H]% 'ht' fala pro lateX para coloar a figura aqui ou no topo da pagina
     \centering % centraliza a figura
      \begin{tikzpicture}[node distance=3cm] % specifies the minimum distance between two nodes. Change if necessary.
          \node[state, initial](q0) {$q_0$};
          \node[state, accepting] at (1,5,2) (q1) {$q_1$};
          \node[state, right = of q0] (q2) {$q_2$};
          
          \draw
            (q0) edge[above] node{$b$} (q2)
            (q0) edge[bend left,above] node{$a$} (q1)
            (q1) edge[out = 140,loop above] node{$a$} (q1)
            (q1) edge[loop above] node{$b$} (q1)
            (q2) edge[bend right,above] node{$\epsilon$} (q1);
      \end{tikzpicture}   
